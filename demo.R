...
reference = read.table("mutrate.txt", sep = "\t", header = TRUE, na.strings = ".")
blacklist <- read.table("GENCODEV19_blacklist.txt", sep = "\t", header = F)
reference = reference[!reference$GeneID %in% blacklist$V1,]

geneset = unique(reference$GeneID)
# load mouse heart dev files
mouse_dev = read.table('mouse_E14.5_heart.txt', header = TRUE, fill = TRUE)
