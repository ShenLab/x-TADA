calculateFDR <- function(pars,
  ...
  B = pars$B
  C = pars$C
  rankPercentile = pars$rankPercentile
  ...
  bfAlls = bfAll
  bfRankPercentile <- matrix(1, nrow = dim(dnData)[1], ncol = 1)
  for (ii in 1:dim(dnData)[1]) {
    bfRankPercentile[ii, 1] <- (C + L / (1+exp(-A*(rankPercentile[ii]-B)))) / 
      (1 + pars$pi0/(1-pars$pi0)*(1-C-L/(1+exp(-A*(rankPercentile[ii]-B)))))
  }
  
  bfAlls <- bfAlls * as.vector(bfRankPercentile)
